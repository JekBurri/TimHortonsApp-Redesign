...
export default function Header() {
  ...
  const totalCount = view.cart.reduce(
  ...
  return (
    <div className="flex justify-between top-0 red-espresso text-white p-2 px-6">
      ...
      <div className="flex gap-2 justify-center my-2">
        <button onClick={() => {
            setView((prevView:any) => ({ ...prevView, page: "cart" }))
        }} className="rounded-lg p-2 border font-bold">
          Cart <span>{totalCount > 0 ? "(" + totalCount + ")" : ""}</span>
        </button>
      </div>
    </div>
  );
